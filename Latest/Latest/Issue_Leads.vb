...
Public Class Issue_Leads
    ...
    Public Sub New(ByVal setup As Boolean, ByVal growshrink As String)
        ...
        Try
            ...
            While R1.Read
                ...
                lblName.Size = New System.Drawing.Size(x, 48)
                ...
                lblAddress.Size = New System.Drawing.Size(x, 48)
                ...
                lblAddress.Anchor = CType((System.Windows.Forms.AnchorStyles.Left Or System.Windows.Forms.AnchorStyles.Right), System.Windows.Forms.AnchorStyles)
                ...
                lblProduct.Size = New System.Drawing.Size(x, 48)
                ...
                ttStatus.SetToolTip(pctRep2, "Add a 2nd Rep")
    ...
    Private Sub Add_Rep(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If i.Tag = "minus" Then
            ...
            i.Tag = "Add"
            ttStatus.SetToolTip(i, "Add a 2nd Rep")
        Else
            ...
            i.Tag = "minus"
            ttStatus.SetToolTip(i, "Close 2nd Rep")
        End If
    End Sub
    Private Sub Pop_Reps(ByVal cbo As ComboBox)
        ...
        cbo.Items.Add("")
        Dim rep_cnx As SqlConnection = New SqlConnection(STATIC_VARIABLES.Cnn.ToString)
        rep_cnx.Open()
        Dim cmdGET As SqlCommand = New SqlCommand("SELECT * from tblTestEmployee Where Department = 'Sales';", rep_cnx)
        Dim rep_r As SqlDataReader = cmdGET.ExecuteReader
        Dim arRep_LIST As New List(Of USER_LOGICv2.Employee)
        While rep_r.Read
            Dim g As New USER_LOGICv2.Employee
            g.ID = rep_r.Item("ID")
            g.FName = rep_r.Item("FName")
            g.LName = rep_r.Item("LName")
            g.MName = rep_r.Item("MName")
            g.ScreenPositionX = rep_r.Item("ScreenPositionX")
            g.ScreenPositionY = rep_r.Item("ScreenPositionY")
            g.ScreenWidth = rep_r.Item("ScreenWidth")
            g.ScreenHeight = rep_r.Item("ScreenHeight")
            g.EmployeeID = rep_r.Item("EmployeeID")
            g.LastForm = rep_r.Item("LastForm")
            g.LastQuery = rep_r.Item("LastQuery")
            g.LastLogon = rep_r.Item("LastLogin")
            g.HostMachineName = My.Computer.Name.ToString
            g.HostMachineIPV4 = rep_r.Item("HostMachineIPV4")
            g.LastPasswordUsed = rep_r.Item("LastPasswordUsed")
            g.CurrentPassword = rep_r.Item("CurrentPassword")
            g.Department = rep_r.Item("Department")
            g.CanEnterLead = rep_r.Item("CanEnterLead")
            g.CanViewAdmin = rep_r.Item("CanViewAdmin")
            g.CanViewPrint = rep_r.Item("CanViewPrint")
            g.CanViewIssueLeads = rep_r.Item("CanViewIssueLeads")
            g.CanPrint = rep_r.Item("CanPrint")
            g.CanBrowseInternet = rep_r.Item("CanBrowseInternet")
            g.CanViewPhoneNumber = rep_r.Item("CanViewPhoneNumber")
            g.LoggedOn = rep_r.Item("LoggedOn")
            g.ColdCall = rep_r.Item("ColdCall")
            g.WarmCall = rep_r.Item("WarmCall")
            g.PreviousCust = rep_r.Item("PreviousCust")
            g.RecoveryLeads = rep_r.Item("RecoveryLeads")
            g.Confirmer = rep_r.Item("Confirmer")
            g.SalesManager = rep_r.Item("SalesManager")
            g.MarketingManager = rep_r.Item("MarketingManager")
            g.Finance = rep_r.Item("Finance")
            g.Install = rep_r.Item("Install")
            g.Administration = rep_r.Item("Administration")
            g.LIC_KEY = rep_r.Item("LIC_KEY")
            g.LEASE_KEY = rep_r.Item("LEASE_KEY")
            g.Has_Email = rep_r.Item("HasEmail")
            g.Has_SMS = rep_r.Item("HasSMS")
            g.PhoneNumber = rep_r.Item("PhoneNumber")
            g.EmailAddress = rep_r.Item("EmailAddress")
            arRep_LIST.Add(g)
            cbo.Items.Add(rep_r.Item("FName") & " " & rep_r.Item("LName"))
        End While
        Me.ListOfReps = arRep_LIST
        rep_r.Close()
        rep_cnx.Close()
        rep_cnx = Nothing
        cnn2.Close()
...
#Region "Get Marekting Result For btnCCIssue Toggle"
    ...
    Private Function GetMResult(ByVal LeadNum As String)
        Dim res As String = ""
        Dim m_resCNX As SqlConnection = New SqlConnection("SERVER=192.168.1.2;Database=ISS;User Id=sa;Password=spoken1;")
        m_resCNX.Open()
