...
#Region "Undo Called and Cancelled"
    ...
    Public Sub Log_CandC_Undone(ByVal LeadNum As String, ByVal TriggerDate As String)
        ...
        Dim m_resCNX As SqlConnection = New SqlConnection("SERVER=192.168.1.2;Database=ISS;User Id=sa;Password=spoken1;")
        m_resCNX.Open()
        Dim cmdUP As SqlCommand = New SqlCommand("UPDATE enterlead SET MarketingResults = 'Confirmed' WHERE ID = '" & LeadNum & "';", m_resCNX)
        cmdUP.ExecuteScalar()
        m_resCNX.Close()

        m_resCNX.Open()
        Dim cmdINS As SqlCommand = New SqlCommand("INSERT leadHistory (LeadNum,GeneratedDate,TriggerDate,Description,Department,SubDepartment)" _
                                                  & "values('" & LeadNum & "','" & TriggerDate & "','" & TriggerDate & "','Called in to so and so and decided to call and cancel appointment.', 'Marketing','LoggedCorrespondence');", m_resCNX)
