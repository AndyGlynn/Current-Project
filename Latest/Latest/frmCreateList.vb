...
Public Class frmCreateList
    ...
    Private Sub rdoCity_CheckedChanged(sender As Object, e As EventArgs) Handles rdoCity.CheckedChanged
        ...
        Me.lblStateSelection.Visible = True
    ...
    Private Sub rdoZip_CheckedChanged(sender As Object, e As EventArgs) Handles rdoZip.CheckedChanged
        ...
        Me.lblStateSelection.Visible = False
    ...
    Private Sub btnGenerate_Click(sender As Object, e As EventArgs) Handles btnGenerate.Click
        ...
        If Me.dpGenerated.Value.ToShortDateString <> "1/1/1900" Then
            Where = "LeadGeneratedOn >= '" & Me.dpGenerated.Value.ToString & "'"
            Fby += 1
        End If
        If Me.dpFrom.Value.ToShortDateString <> "1/1/1900" Then
            If Where <> "" Then
                Where = Where & " and "
            End If
            Where = Where & "ApptDate Between '" & Me.dpFrom.Value.ToString & "' and '" & Me.dpTo.Value.ToString & "'"
            Fby += 1
        End If
        If Me.tpFrom.Value.ToShortTimeString <> "12:00 AM" Then
            If Where <> "" Then
                Where = Where & " and "
            End If
            Where = Where & "ApptTime Between '" & Me.tpFrom.Value.ToString & "' And '" & Me.tpTo.Value.ToString & "'"
            Fby += 1
        End If
        If Me.chWeekdays.Checked = True Then
            If Where <> "" Then
                Where = Where & " and "
            End If
            Where = Where & "ApptDay <> 'Saturday' and ApptDay <> 'Sunday'"
            Fby += 1
        End If
        ''Marketer Tab 
        Dim cnn As SqlConnection = New SqlConnection(STATIC_VARIABLES.Cnn)
        ...
        If r1.Item(0) > Me.chlstMarketers.Items.Count Then    '' If true were working with a filtered down set of Marketers 
            ...
            For x As Integer = 0 To Me.chlstMarketers.CheckedItems.Count - 1
                ...
                If x <> Me.chlstMarketers.CheckedItems.Count - 1 Then
                    ...
                    Where = Where & "Marketer = '" & Me.chlstMarketers.CheckedItems(x).ToString & "' and "    ''Add Comma after each item until last item 
                    Fby += 1
                Else
                    Where = Where & "Marketer = '" & Me.chlstMarketers.CheckedItems(x).ToString & "')"       ''Last Item Dont Add comma to end of where 
                    Fby += 1
                End If
            Next
        Else
            If Me.chlstMarketers.Items.Count <> Me.chlstMarketers.CheckedItems.Count Then  ''If all items are checked in list checkbox and were not working with a filtered down marketer list then no need to query by marketer at all
                ...
                For x As Integer = 0 To Me.chlstMarketers.CheckedItems.Count - 1
                    ...
                    If x <> Me.chlstMarketers.CheckedItems.Count - 1 Then
                        ...
                        Where = Where & "Marketer = '" & Me.chlstMarketers.CheckedItems(x).ToString & "' or "    ''Add Comma after each item until last item 
                        Fby += 1
                    Else
                        Where = Where & "Marketer = '" & Me.chlstMarketers.CheckedItems(x).ToString & "')"       ''Last Item Dont Add comma to end of where 
                        Fby += 1
        ...
        If Me.chlstProducts.Items.Count <> Me.chlstProducts.CheckedItems.Count Then ''If true filter by checked products, if false no need to filter products (skip to next step)
            ...
            For x As Integer = 0 To Me.chlstProducts.CheckedItems.Count - 1
                ...
                If x <> Me.chlstProducts.CheckedItems.Count - 1 Then
                    ...
                    Where = Where & "(" & "Product1 = '" & Me.chlstProducts.CheckedItems(x).ToString & "' or Product2 = '" & Me.chlstProducts.CheckedItems(x).ToString & "' or Product3 = '" & Me.chlstProducts.CheckedItems(x).ToString & "') or "    ''Add Comma after each item until last item 
                    Fby += 1
                Else
                    Where = Where & "(" & "Product1 = '" & Me.chlstProducts.CheckedItems(x).ToString & "' or Product2 = '" & Me.chlstProducts.CheckedItems(x).ToString & "' or Product3 = '" & Me.chlstProducts.CheckedItems(x).ToString & "'))"       ''Last Item Dont Add comma to end of where 
                    Fby += 1
                End If
            Next
        End If
        ...
        If Me.chlstZipCity.CheckedItems.Count > 0 Then
            ...
            For x As Integer = 0 To Me.chlstZipCity.CheckedItems.Count - 1
                ...
                If x <> Me.chlstZipCity.CheckedItems.Count - 1 Then
                    ...
                    Where = Where & CorZ & " = '" & Me.chlstZipCity.CheckedItems(x).ToString & "' or "    ''Add Comma after each item until last item 
                    Fby += 1
                Else
                    Where = Where & CorZ & " = '" & Me.chlstZipCity.CheckedItems(x).ToString & "')"       ''Last Item Dont Add comma to end of where 
                    Fby += 1
                End If
            Next
        End If
        ...
        If Me.chkWC.Checked = True Then
            ...
            For x As Integer = 0 To Me.chlstWC.CheckedItems.Count - 1
                ...
                If x <> Me.chlstWC.CheckedItems.Count - 1 Then
                    ...
                    Where = Where & "(" & Field & " = '" & Me.chlstWC.CheckedItems(x).ToString & "' and isRecovery = 'False') or "    ''Add Comma after each item until last item 
                    Fby += 1
                Else
                    Where = Where & "(" & Field & " = '" & Me.chlstWC.CheckedItems(x).ToString & "' and isRecovery = 'False'))"       ''Last Item Dont Add comma to end of where 
                    Fby += 1
                End If
            Next
        End If
        ...
        If Me.chRehash.Checked = True Then
            ...
            For x As Integer = 0 To Me.chlstRehash.CheckedItems.Count - 1
                ...
                If x <> Me.chlstRehash.CheckedItems.Count - 1 Then
                    ...
                    Where = Where & "(" & Field & " = '" & Me.chlstRehash.CheckedItems(x).ToString & "' and isRecovery = 'True' and (QuotedSold *" & Per & ") >= ParPrice and QuotedSold >= " & Q & " ) or "    ''Add Comma after each item until last item 
                    Fby += 1
                Else
                    Where = Where & "(" & Field & " = '" & Me.chlstRehash.CheckedItems(x).ToString & "' and isRecovery = 'True' and (QuotedSold *" & Per & ") >= ParPrice and QuotedSold >= " & Q & "))"       ''Last Item Dont Add comma to end of where 
                    Fby += 1
                End If
            Next
    ...
    Private Sub btnClear_Click(sender As Object, e As EventArgs) Handles btnClear.Click
        Me.epForm.Clear()
        Me.epGeo.Clear()
        Me.dpGenerated.Value = "1/1/1900 12:00:00"
        Me.txtGenerated.Visible = True
        Me.dpFrom.Value = "1/1/1900 12:00:00"
        Me.txtFrom.Visible = True
        Me.dpTo.Value = "1/1/2100 12:00:00"
        Me.txtTo.Visible = True
