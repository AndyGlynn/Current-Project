...
Public Class VerifyAddress
    ...
    Public Class ADHOC_VERIFY_ADDRESS
        Public oMap As MapPoint.Map
        Public oResults As MapPoint.FindResults
        Public oRoute As MapPoint.Route
        Public oWayPoints As MapPoint.Waypoints
        ...
        Private cnn As SqlConnection = New SqlConnection(STATIC_VARIABLES.Cnn)
        Public Sub New(ByVal StAddress As String, ByVal City As String, ByVal State As String, ByVal Zip As String, ByVal RunCnt As Integer, ByVal ID As String)
            ...
            ''
            ArValues = New ArrayList
            FilterX(StAddress, City, State, Zip, ID)

        End Sub
        Public Sub FilterX(ByVal StAddress As String, ByVal City As String, ByVal State As String, ByVal Zip As String, ByVal ID As String)
            Try
                If Me.RunCnt = 4 Then
                    Exit Sub
                End If
                ...
                Select Case Me.ArValues.Count  ''count of actual valid addresses
                    ...
                    Case Is = 1
                        ...
                        cnx.Open()
                        Dim strUP As String = "UPDATE EnterLead" & vbCrLf
                        strUP += "SET StAddress = '" & StAddress & "', " & vbCrLf
                        strUP += " City = '" & City & "', " & vbCrLf
                        strUP += "State = '" & State & ", " & vbCrLf
                        strUP += "Zip = '" & Zip & "';"
                        Dim cmdUP As New SqlCommand(strUP, cnx)
                        cmdUP.ExecuteReader()
                        cnx.Close()

                        Dim cmdINS As String = "INSERT VerifiedAddress (LeadNum,Verified) values('" & ID & "',1);"
                        cnx.Open()
                        Dim cmd_I As New SqlCommand(cmdINS, cnx)
                        cmd_I.ExecuteReader()
                        cnx.Close()
